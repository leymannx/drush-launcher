...
In order to avoid dependency issues, it is best to require WP-CLI on a per-project basis via Composer (`composer require drush/drush`). This makes WP-CLI available to your project by placing it at `vendor/bin/wp`.
...
1. Download latest stable release via CLI (code below) or browse to https://github.com/leymannx/wp-cli-launcher/releases/latest.
    ...
    OSX:
    ```Shell
    curl -OL https://github.com/leymannx/wp-cli-launcher/releases/download/0.2.0/wp-cli.phar
    ```

    Linux:

    ```Shell
    wget -O wp-cli.phar https://github.com/leymannx/wp-cli-launcher/releases/download/0.2.0/wp-cli.phar
    ```
...
1. Export an environment variable: `export WP_CLI_LAUNCHER_FALLBACK=/path/to/drush`
1. Specify an option: `--fallback=/path/to/drush`

## Xdebug compatibility

WP-CLI Launcher, like Composer automatically disables Xdebug by default. This improves performance substantially. You may override this feature by setting an environment variable. ``WP_CLI_ALLOW_XDEBUG=1 drush [command]``
